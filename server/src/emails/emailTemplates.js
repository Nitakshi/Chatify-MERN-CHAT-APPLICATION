export function createWelcomeEmailTemplate(name, clientURL) {
  const escapeHtml = (s) => 
    String(s).replace(/&/g, "&amp;") //prevent XSS attacks by escaping special characters
     .replace(/</g, "&lt;")
     .replace(/>/g, "&gt;")
     .replace(/"/g, "&quot;")
     .replace(/'/g, "&#039;");
    const safeName = escapeHtml(name ?? ""); // Handle null or undefined name
    const safeClientURL = escapeHtml(clientURL ?? "#"); // Handle null or undefined clientURL
  return `
  <!DOCTYPE html>
  <html lang="en">
  <head>
